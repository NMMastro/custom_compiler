...
int main() {
  ...
  while (in >> s) {
    ...
    bool rejected_in_loop = false;

    for ( char c : s) {
        std::pair key = std::make_pair(currentState, c);

        if (transitions.find(key) != transitions.end()) {
            currentState = transitions[key];
        }
        else {
            rejected_in_loop = true;
            break;
        }
    }
    ...
    if (accepting_states.count(currentState) > 0 && !rejected_in_loop) {
