...
string type_expression(Node* node) {
    ...
    if (node->value == "factor STAR factor" || node->value == "lvalue STAR factor") {
        string factorType = type_expression(node->children[1]);
    ...
    if (node->value == "factor AMP lvalue") {
        string lvalueType = type_expression(node->children[1]);
    ...
    if (node->value == "factor NEW LONG LBRACK expr RBRACK") {
        string exprType = type_expression(node->children[3]);
    ...
    if (node->value == "term term STAR factor" ||
        ...
        string leftType = type_expression(node->children[0]);
        string rightType = type_expression(node->children[2]);
