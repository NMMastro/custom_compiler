...
// x19 -> currentPosition
// x20 -> endCode
// x21 -> beginning of stored program
// x22 -> current location in stored program
// x23 -> end of stored program
// x24 -> end of module
...
lastHalfWord:
    blr x2 // read halfword
    blr x3 // print halfword
    stur x0, [x22, 0]
    add x22, x22, x8

endFirstLoop:
    add x23, x22, xzr // end of stored program
    add x19, x20, xzr // set x19 to end of code
...
relocationLoop:
    ...
    // Read format code (will always be 1)
    blr x1
    add x19, x19, x8
    ...
    // Read the address stored in x5
    ldur x6, [x5, 0]

    // Relocation adjustment: alpha - 20
    sub x7, x21, x4
    add x6, x6, x7
...
endRelocation:
    add x22, x21, xzr // reset current pointer
...
// stored sp in local storage
add x0, sp, xzr

ldr x4, 8
b 12
.8byte spSaveLocation
