...
// g++ -g -O0 -fPIC -std=gnu++20 -Wall -Wextra -pedantic-errors -Wno-unused-parameter -o wlp4parse wlp4parse.cc
...
void perform_shift() {
    ...
    Node* leaf = new Node;
    leaf->value = token.kind + " " + token.lexeme;
...
int main() {
    ...
    // CFG input section
    stringstream data(WLP4_COMBINED);
    getline(data, line);
    ...
    while (getline(cin, line)) {
        ...
        stringstream ss(line);
        while (ss >> t.kind >> t.lexeme) {
            input.push_back(t);
        }
    }

    input.push_back(Token{"EOF", "EOF"});
    ...
    while (true) {
        ...
        if (reductions.count({current_state, next}) > 0) {
            ...
            if (input.front().kind != "BOF" && input.front().kind != "EOF") {
                shifted++;
            }
            perform_shift();
